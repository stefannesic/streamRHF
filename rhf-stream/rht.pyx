...
def rht(float[:,:] data, int[:,:] indexes, insertionDS, split_info, float[:,:,:] moments, int start, int end, int nd, int H, int nodeID=0):
    ...
    if (end == start or nd >= H):
        # leaf size
        indexes = fill_leaf(indexes, insertionDS, nodeID, nd, H, start, end) 
    else:
        # calculate kurtosis
...
def fill_leaf(int[:,:] indexes, insertionDS, int nodeID, int nd, int H, int start, int end):
    cdef int ls, i, leaf_index
    ls = end - start + 1

    # calculate index of insertionDS 
    if nd >= (2**H - 1) : # leaf is at max depth
        leaf_index = nodeID
    else:
        leaf_index = 2**(H-nd)*nodeID + 2**(H-nd) - 1
    
    # at this point, leaf_index is nodeID and needs to be adjusted for indexing
    leaf_index = nodeID - (2**H)
