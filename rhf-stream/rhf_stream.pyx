...
cpdef rhf_stream(double[:,::1] data, int t, int h, int N_init_pts):
    ...
    for i in range(t):
        ...
        for j in range((2**h) - 1):
            ...
            # r0 and r1 cannot be equal to the lower bound
            while r0 == 0:
...
cdef (int, double, double, double) get_attribute(double[:,::1] data, int[:] indexes, Py_ssize_t start, 
                                        Py_ssize_t end, double ks, double[:] kurt, int d, 
                                        double r_a=-1, double r_a_val=-1):
    ...
    return a, a_val, r_a / ks, ((r_a_val - a_min) / (a_max - a_min))
